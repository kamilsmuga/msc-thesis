...
This metric represents distance from intersection points calculated based on uptime. Values can be negative to indicate non-optimal profile or positive otherwise. There are cases when it is better to be either below or above a threshold. It will be specified by flag parameter calculated together with \textit{ip(scheme1, scheme2)} results. Algorithm~\ref{alg:distance_from_optimality} represents pseudo code implementation.

\begin{algorithm}[H]
 \label{alg:distance_from_optimality}
 \KwData{Uptime, Intersection Point, Spin;}
...
\begin{enumerate}
\item Input: Uptime: 24, Optimality Point: 18, Spin: True (more optimal for values above threshold). Output: +6
\item Input: Uptime: 16, Optimality Point: 18, Spin: True (more optimal for values above threshold). Output: -2
\item Input: Uptime: 15, Optimality Point: 18, Spin: False (more optimal for values below threshold). Output: +3
\item Input: Uptime: 19, Optimality Point: 18, Spin: False (more optimal for values below threshold). Output: -1
\end{enumerate}
...
\ForAll{machine in Other Machines}{
        \If{$machine.cpu < cluster.cpu$} {
                 $spare_cpu  clustercpu  machinecpu$
        }
        %\EndIf
}
%\EndFor
