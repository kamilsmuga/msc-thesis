...
\textsl{New Infrastructure as a Service solutions are becoming available with a growing number of supported pricing models. A hosted Cloud environment is a strong choice to design and build an infrastructure for a product. The recent availability of different pricing schemes based on resource utilization and uptime reveals new challenges in an already complex capacity planning process. There is a choice between ad-hoc provisioning and time commitment that comes with reduced hourly rates. When one decides for a time commitment, there are still more pricing options available. Few different prices for the same resource that depend on a daily uptime. Which one is the best one? When exactly does one pricing scheme becomes more cost effective that the other? We attempted to answer these critical questions by i) aggregating and transforming metrics data, ii) designing key optimality indicators, iii) using the indicators to build algorithms that suggest better configuration options, iv) using an existing large dataset to evaluate the algorithms. }

\end{abstract}
\newpage
\chapter*{Acknowledgements}

I would like to thank Prof. Liam Murphy for help and guidance.

I thank Christina Thorpe for many insightful conversations during the development of the ideas in this thesis. And for her time that I should use more. 

I also wish to express my gratitude to Alan for inspiration to do this research.

I owe more than thanks to my lovely Klaudia for her support and constant encouragement during the entire process.

And finally, but not least, thanks goes to my parents who have always motivated me to achieve more.
...
Most of the related work is conducted from an IaaS provider perspective~\cite{6274129}~\cite{6253563} or aimed to find an optimal pricing state for providers and consumers to collaborate~\cite{6676685}~\cite{6963393}. This thesis is different in a way that it analyses a SaaS provider capacity planning in an environment when one resource has different prices tightly coupled with an uptime.

%%        *Describe the proposed solution

Our proposed solution implements the objectives using mainly map-reduce jobs executed on Apache Spark\footnote{\url{https://spark.apache.org}} platform. Aggregations, key indicators and algorithms were designed, implemented in pseudo-code and Python and evaluated on Google cluster-usage trace~\cite{clusterdata:Reiss2011}. 

%%        *Mention some of the challenges

Main challenges include dealing with dynamic changes of pricing schemes offered by IaaS providers. Even though this thesis uses Amazon's related taxonomy, it should be read as an attempt to solve a general case for uptime-based variable pricing capacity planning. Another challenging aspect is a cost of migration to suggested environment. It was not considered in the results, however, it created an opportunity for future work to investigate an impact of the delta difference between an old and new configuration cost. Lastly, we encountered technical issues related to implementation and execution of aggregations and algorithms on large data sets. Jobs were failing due to not enough of RAM capacity on a single machine. The issues were resolved by a combination of code improvements, like changing the way aggregations build hashmaps and an increase of parallelism and chunking granularity on the platform.
...
\textbf{Background}. This section outlines the background information related to IaaS consumer's point of view and challenges related to finding the best configuration for the money. \par
\textbf{Design and Implementation}. This section explains the design and environmental conditions for the aggregations, key indicators and algorithms. It also dives into implementation details on the Apache Spark platform. \par
...
IaaS provider's pricing can be flexible as described in below sections. One instance configuration can have many prices based on a daily uptime. SaaS provider faces the challenge to pick the best one.
...
\paragraph{Spot instances.} A real-time bidding platform. A customer can bid on the price. An instance can run only when the bid exceeds current Spot Price.
\paragraph{Reserved instances.} Discounted price in comparison to On-demand in exchange for 1 or 3 years of commitment. Guaranteed availability of reserved capacity. 

This thesis focuses around the concept of Reserved Instances. The scheme is split further into Light, Medium and Heavy options. They differ in upfront and hourly costs. Based on pricing data, Light serves well for short periods of daily usage, Heavy is optimized for long workloads and Medium is in between. \\
...
\section{Taxonomy}
This section explains a terminology being used for further design and evaluation stages. It is important to redefine some terms, like cost per hour, which is sometimes presented by IaaS providers in a non-complete way and may not include all costs. We also define new terminology that will be used throughout this thesis.
...
This section contains design and implementation details for an end-to-end approach that analyzes machine configuration and proposes changes.
We start with metrics data prerequisites and guide through data transformations. We further define key indicators and formalize their implementations. The indicators can be used to filter non-optimal configurations. Finally, we present algorithms that take the filter data as an input and generate configuration suggestions.
...
The following list presents required metrics.
...
Metrics should be aggregated in \textless K, List\textless V\textgreater\textgreater format where K represents Machine Id and List of V is a list of measurements for a given period. 
...
The algorithm takes upfront cost, charge per hour and daily uptime as input parameters. It aggregates both costs and calculates a real (total) cost per hour~\textit{if} a machine were to run every day for a number of previously specified hours. We use this algorithm to build lookup tables for a 24-hour usage period. Essentially, we pass the same pricing scheme parameters and specify daily uptime from 1 to 24. This creates a set with hours per day to~\textit{cph} mapping.
...
At 18 hours, the point of scheme cost intersection (highlighted as 1), it does not matter which scheme is chosen as the cost is the same. This is point 0.
...
A user can define which KDI value is non-optimal and run a report to find machines below this point. For example, every negative KDI value signalizes there is a better pricing scheme available. A user can catch this situation by having a threshold value set to 0. However, there might be a situation when a machine is taken for maintenance and reports negative values for one or a couple of days. To avoid this kind of background noise, a user can add an another criterion to specify a minimum number of days of reported values below the threshold.
...
This approach has a potential to perform well in under-utilized clusters. For well-balanced environments with similar usage patterns, it does not find a lot of spare capacities and would propose fairly limited improvements. \\
...
\label{alg:squeeze-them-in}
 ...
 \begin{algorithmic}[1]
  \State $float spareCpu = 0;$
  \State $float spareMemory = 0;$
  \State $ID[ ] machinesToExclude;$
  \State \ForAll{machine in otherMachines}{
  ...
  \State \ForAll{machine in machinesFromReport}{
	  \If{$(spareCpu - machine.cpu > 0) \&\& (spareMemory - machine.memory > 0)$}{
	                $add machine to machinesToExclude;$
	        }
	}
\State\Return machinesToExclude;
\end{algorithmic}
\end{algorithm}

The algorithm takes CPU and memory cluster averages, a list of machines to optimize and list of the other machines as an input. First, it calculates the amount of spare capacity based on other machines data (code block 4). The results are saved in memory using variables listed in line 1 and 2. Then, it iterates through all of the machines that were identified to be non-optimal (code block 5) and if there is enough spare capacity it adds them to the exclusion list (line 3). Finally, it returns a list of machines that can be powered off. 

\subsubsection{New capacity configuration suggestion}

The new configuration is calculated based on input from the report, list of available schemes and total uptime to handle. Pseudo-code implementation is presented in Algorithm~\ref{alg:new_configuration}. The algorithm iterates through all of the available schemes, reads \textit{intersection points} and gets \textit{cph} for each of the \textit{ip}s. It returns a total cost and number of machines required for each proposed configuration. \\
...
\label{alg:new_configuration}
 ...
 \begin{algorithmic}[1]
  \State $float\{\} totalCost;$
  \State\ForAll{scheme in availableSchemes}{
		\ForAll{ip in scheme.intersectionPoints}{
			$vms = Uptime / ip.getHours()$ \par
			$cost = ip.getCphForHours(ip.getHours())$ \par
			$totalCost[ip] = (vms, cost)$\par
		}
}
...
The algorithm takes a list of machines from the report, list of available schemes and total uptime to accommodate. It iterates through available schemes (code block 2) and for each of them attempts to suggest a configuration for a given uptime. The suggestion process is split into phases 1) calculate a number of VMs required to handle requested uptime -  VM's uptime time depends on \textit{intersection point} value, 2) calculate cost of running the VM based on a corresponding \textit{cph} value, 3) add results to totalCost map (line 1). Finally, the algorithm returns a map of total cost and VMs required for each analyzed scheme.
...
\item validation of a correlation between \textit{KDI} and \textit{total cost} changes.
\end{enumerate}
...
Amazon Web Services\footnote{\url{http://aws.amazon.com/}} offered variable pricing schemes based on uptime. Customers could choose an instance configuration and the total cost would be calculated based on a number of hours of uptime per day.
...
Table~\ref{tab:new_schema} is a new schema produced from a join of capacity metrics from machine events and aggregations computed from tasks events data. 
...
\begin{center}
    ...
    \end{tabular}
\end{center}
\caption{Results of the report run on the cluster with criteria of KDI below 0 for 29 days}
